...
Sessions now carry `title`, `options`, `criteria` (`[id, label, importance]`), `evals` (`{pref: -2..2, conf: 1..5}` for two options; `{ratings: {label: 1..5}, conf}` for more), optional `notes` (`hope`, `stillWant`, `worry`) and `ruledOut`. The engine computes weights, expected utility, sensitivity (the swing needed to reorder the top two), uncertainty and a hinge score; the Pivot is the top hinge the person hasn't ruled out. Invariants added: the rendered report contains no digits or percent signs; the word Pivot never appears when the verdict is "none" or "robust". Persona evaluations v1 and v2 were run against the earlier seven-question engine and are kept for history. v3 (`evaluation-v3.md`) ran the same eleven people through this model and found the hope test pre-empting larger hinges, a direction-ambiguous flip sentence, a term clash in "Less important", a missing heavy-decision branch, list-order hinge ties, and silent dominated options; all fixed and covered by scenarios.
